...
public:
  ...
  private_const_view(const private_const_view& v) :
    preview<Container>(v.array, v.x, v.nx),
    cache(v.cache)
  {
...
protected:
  ...
  // inspector
  value_type get(size_t x) const { return cache.get(x, this->stream); }
...
protected:
  ...
  // mutator
  void set(size_t x, value_type val) { cache.set(x, val, this->stream); }

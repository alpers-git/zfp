...
public:
  ...
  // inspector
  Scalar get(size_t i, void* stream = 0) const
  {
    const CacheLine* p = line(i, false, stream);
    return (*p)(i);
  }

  // mutator
  void set(size_t i, Scalar val, void* stream = 0)
  {
    CacheLine* p = line(i, true, stream);
    (*p)(i) = val;
  }
...
protected:
  ...
  CacheLine* line(size_t i, bool write, void* stream = 0) const
  {
    CacheLine* p = 0;

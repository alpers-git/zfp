#ifndef SYCL_ZFP_ENCODE_H
#define SYCL_ZFP_ENCODE_H
...
{
  return sycl::min(maxprec, sycl::max(0, maxexp - minexp + 8));
}
...
exponent(Scalar x)
{
  ...
  if (x > 0) {
    ...
    e = sycl::max(e, 1 - get_ebias<Scalar>());
  }
#endif
  return e;
}
...
encode_block exceeds 128 bytes and may cause high register pressure. Consult
with your hardware vendor to find the total register size available and adjust
the code, or use smaller sub-group size to avoid high register pressure.
*/
void inline encode_block(BlockWriter<BlockSize> &stream, int maxbits,
                         int maxprec, Int *iblock, unsigned char *perm_3d,
                         unsigned char *perm_1, unsigned char *perm_2)
{
...
zfp_encode_block exceeds 128 bytes and may cause high register pressure. Consult
with your hardware vendor to find the total register size available and adjust
the code, or use smaller sub-group size to avoid high register pressure.
*/
void inline zfp_encode_block(Scalar *fblock, const int maxbits,
                             const uint block_idx, Word *stream,
